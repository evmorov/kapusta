...
def describe(x)
  (-> do
    kap_case_value_1 = x
    kap_match_8 = kap_match_pattern([:lit, 0], kap_case_value_1)
  if kap_match_8[0]
    kap_bindings_9 = kap_match_8[1]
    "zero"
  else
      kap_match_6 = kap_match_pattern([:lit, 1], kap_case_value_1)
    if kap_match_6[0]
      kap_bindings_7 = kap_match_6[1]
      "one"
    else
        kap_match_4 = kap_match_pattern([:bind, "n", false], kap_case_value_1)
      if kap_match_4[0]
        kap_bindings_5 = kap_match_4[1]
        n = kap_bindings_5.fetch("n")
        if (n < 0)
          "negative"
        else
