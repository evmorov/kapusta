...
def loose(v)
  (-> do
    kap_case_value_1 = v
    kap_match_4 = kap_match_pattern([:bind, "_x", true], kap_case_value_1)
  if kap_match_4[0]
    kap_bindings_5 = kap_match_4[1]
    _x = kap_bindings_5.fetch("_x")
  _x
  else
...
def strict(v)
  (-> do
    kap_case_value_6 = v
    kap_match_9 = kap_match_pattern([:bind, "x", false], kap_case_value_6)
  if kap_match_9[0]
    kap_bindings_10 = kap_match_9[1]
    x = kap_bindings_10.fetch("x")
  x
  else
      kap_match_7 = kap_match_pattern([:wild], kap_case_value_6)
    if kap_match_7[0]
      kap_bindings_8 = kap_match_7[1]
      "fallback"
    else
        nil
    end
