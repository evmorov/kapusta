...
def area(shape)
  (-> do
    ...
    kap_match_8 = kap_match_pattern([:vec, [[:lit, :circle], [:bind, "r", false]]], kap_case_value_1)
  if kap_match_8[0]
    kap_bindings_9 = kap_match_8[1]
    r = kap_bindings_9.fetch("r")
  3.14 * r * r
  else
      kap_match_6 = kap_match_pattern([:vec, [[:lit, :square], [:bind, "s", false]]], kap_case_value_1)
    if kap_match_6[0]
      kap_bindings_7 = kap_match_6[1]
      s = kap_bindings_7.fetch("s")
    s * s
    else
        kap_match_4 = kap_match_pattern([:vec, [[:lit, :rect], [:bind, "w", false], [:bind, "h", false]]], kap_case_value_1)
      if kap_match_4[0]
        kap_bindings_5 = kap_match_4[1]
        w = kap_bindings_5.fetch("w")
      h = kap_bindings_5.fetch("h")
      w * h
      else
...
[[:circle, 5], [:square, 3], [:rect, 2, 4], [:dot]].each_with_index do |s, _|
  p area(s)
end
nil
